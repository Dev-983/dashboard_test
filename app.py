...
TEMP_FOLDER = 'temp'
DATA_FILE = os.path.join('templates', 'boxes_data.json')
DATA_FILE_STDYTIMELINE = os.path.join(TEMP_FOLDER, 'study_timeline_data.json')
...
def load_data_21():
    if os.path.exists(DATA_FILE_STDYTIMELINE):
        with open(DATA_FILE_STDYTIMELINE, 'r') as file:
            return json.load(file)
    return []

def save_data_21(data):
    with open(DATA_FILE_STDYTIMELINE, 'w') as file:
...
############################################################################################
@app.route('/studytimeline_test')
def studytimeline_test():
    return render_template('project_study_testpage.html')
